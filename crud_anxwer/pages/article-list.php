...
while($data=mysqli_fetch_assoc($result)) {
    ...
    echo ('<form method="POST" action="pages/manage.php" id="manage'.$data["id"].'">');
    echo ('<input type="hidden" name="id" value="'.$data["id"].'">');
    echo ('</form>');
    echo ('<form method="POST" action="pages/delete.php" id="delete'.$data["id"].'">');
    echo ('<input type="hidden" name="id" value="'.$data["id"].'">');
    echo ('</form>');
    echo ('<td>');
...
function hapus(pid) {
    ...
    if (proceed) {
        $.ajax({
            ...
            url: 'pages/delete.php', // The file where my php code is
            data: {
